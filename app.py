...
TUMOR_INFO = {
    'Glioma': {
        'grade':     'High Grade (HGG) — Grade III/IV',
        'risk':      'HIGH',
        'risk_color':'#FF4444',
        'desc':      'Gliomas arise from glial cells. High-grade variants grow rapidly and require immediate medical attention.',
        'treatment': 'Surgery + Radiation + Chemotherapy',
    },
    'Meningioma': {
        'grade':     'Low Grade (LGG) — Grade I/II',
        'risk':      'MEDIUM',
        'risk_color':'#FFA500',
        'desc':      'Meningiomas arise from meninges. Usually benign but may require monitoring or surgery.',
        'treatment': 'Observation / Surgery',
    },
    'Pituitary': {
        'grade':     'Low Grade (LGG) — Grade I',
        'risk':      'LOW',
        'risk_color':'#FFA500',
        'desc':      'Pituitary tumors affect hormone regulation. Usually benign with good prognosis.',
        'treatment': 'Medication / Surgery',
    },
    'No Tumor': {
        'grade':     'N/A',
...
# ── Occlusion Saliency Map (replaces Grad-CAM) ────────
def get_gradcam_heatmap(img_array, class_idx):
    """
    Occlusion-based saliency — works with TFLite, no .h5 needed.
    Blacks out 14x14 patches and measures prediction drop.
    """
    h, w     = 14, 14
    patch_h  = img_array.shape[1] // h
    patch_w  = img_array.shape[2] // w
    ...
    interpreter.invoke()
    base_score = float(
        interpreter.get_tensor(output_details[0]['index'])[0][class_idx]
    )

    for pi in range(h):
        for pj in range(w):
            occluded = img_array.copy()
            y1, y2   = pi * patch_h, (pi + 1) * patch_h
            x1, x2   = pj * patch_w, (pj + 1) * patch_w
            occluded[0, y1:y2, x1:x2, :] = 0.0

            interpreter.set_tensor(input_details[0]['index'],
                                   occluded.astype(np.float32))
            interpreter.invoke()
            occ_score = float(
                interpreter.get_tensor(output_details[0]['index'])[0][class_idx]
            )
            saliency[pi, pj] = max(0.0, base_score - occ_score)
...
# ── Overlay Heatmap ───────────────────────────────────
def overlay_heatmap(file_path, heatmap):
    orig            = cv2.imread(file_path)
    orig            = cv2.resize(orig, (224, 224))
    ...
    return cv2.addWeighted(orig, 0.6, heatmap_colored, 0.4, 0)

# ── Tumor Size ────────────────────────────────────────
def get_tumor_size(heatmap, threshold=0.5):
    binary     = (heatmap > threshold).astype(np.uint8)
    percentage = (np.sum(binary) / heatmap.size) * 100
    size_label = ("Small"  if percentage < 5  else
                  "Medium" if percentage < 15 else "Large")
    return percentage, size_label
...
def get_tumor_location(heatmap):
    ...
    coords = np.argwhere(binary)
    cy, cx = np.mean(coords[:, 0]), np.mean(coords[:, 1])
    v_loc  = ("Superior (Top)"    if cy < h * 0.33 else
               "Central (Middle)" if cy < h * 0.66 else
               "Inferior (Bottom)")
    h_loc  = ("Left"    if cx < w * 0.33 else
               "Central" if cx < w * 0.66 else
               "Right")
    return f"{v_loc} — {h_loc}"
...
# ── Title ─────────────────────────────────────────────
tk.Label(main_frame,
         text="🧠 Brain Tumor Analysis System",
         font=('Arial', 20, 'bold'),
         bg='#0D1B2A', fg='#00C2CB').pack(pady=15)

tk.Label(main_frame,
         text="ResNet50V2  |  Occlusion Saliency  |  Auto Diagnosis",
         font=('Arial', 10),
         bg='#0D1B2A', fg='#7FB3C8').pack()
...
img_frame = tk.Frame(main_frame, bg='#1B2838', bd=2, relief='groove')
img_frame.pack(pady=12)
img_label = tk.Label(img_frame, bg='#1B2838',
                     text="Upload MRI Image to Begin",
                     fg='#4A6FA5', font=('Arial', 12),
                     width=46, height=15)
img_label.pack(padx=8, pady=8)
...
progress.pack(pady=5)
status_var = tk.StringVar(value="Ready — Upload an MRI image")
tk.Label(main_frame, textvariable=status_var,
...
def make_card(parent, title, color, row, col):
    ...
    tk.Label(card, text=title, font=('Arial', 9, 'bold'),
             bg='#162032', fg=color).pack(pady=(8, 2))
    val = tk.StringVar(value="—")
    tk.Label(card, textvariable=val,
             font=('Arial', 12, 'bold'),
             bg='#162032', fg='white',
             wraplength=170).pack(pady=(2, 8))
    return val
...
tk.Label(info_frame, text="📋 Clinical Information",
         font=('Arial', 11, 'bold'),
         bg='#162032', fg='#F5A623').pack(anchor='w', padx=12, pady=(10, 4))

desc_var = tk.StringVar(value="Upload an MRI image to see detailed analysis.")
tk.Label(info_frame, textvariable=desc_var,
         font=('Arial', 10), bg='#162032', fg='#B8D4E8',
         wraplength=720, justify='left').pack(anchor='w', padx=12, pady=2)

treatment_var = tk.StringVar(value="")
tk.Label(info_frame, textvariable=treatment_var,
         font=('Arial', 10, 'bold'), bg='#162032', fg='#00C2CB',
...
def upload_and_predict():
    ...
    try:
        ...
        # ── Step 1: Predict ───────────────────────────
        raw_class, confidence, class_idx = tflite_predict(img_array)

        # ── Step 2: Saliency map ──────────────────────
        status_var.set("🔄 Generating Saliency Map... (~5 sec)")
        root.update()
        heatmap = get_gradcam_heatmap(img_array, class_idx)

        # ── Step 3: Map class name ────────────────────
        name_map = {
            'glioma':     'Glioma',
        ...
        # ── Step 4: Tumor size & location ────────────
        size_pct, size_label = get_tumor_size(heatmap)
        location = (get_tumor_location(heatmap)
                    if class_name != 'No Tumor' else "N/A")
        ...
        overlay = overlay_heatmap(file_path, heatmap)
        ...
        axes[1].imshow(cv2.cvtColor(heat_colored, cv2.COLOR_BGR2RGB))
        axes[1].set_title('Saliency Heatmap', color='white', fontsize=13)
        axes[1].axis('off')

        title = (f"Type: {class_name}  ({confidence * 100:.1f}%)\n"
                 f"Grade: {info['grade'][:22]}\n"
                 f"Risk: {info['risk']}  |  "
                 f"Size: {size_label} ({size_pct:.1f}%)\n"
                 f"Location: {location[:35]}")
        axes[2].imshow(cv2.cvtColor(overlay, cv2.COLOR_BGR2RGB))
        axes[2].set_title(title, color='white', fontsize=10)
        axes[2].axis('off')
        ...
        plt.tight_layout()
        plt.savefig(r'C:\brain-tumor-project\gradcam_result.png',
                    facecolor='#0D1B2A')
        plt.show()
...
tk.Label(main_frame,
         text="Saliency map visualization will open in a separate window",
         font=('Arial', 9),
         bg='#0D1B2A', fg='#4A6FA5').pack(pady=(0, 15))
